...
DATA=$(curl -s "https://graph.instagram.com/v18.0/${INSTAGRAM_USER_ID}/media?fields=id,caption,media_type,like_count,comments_count,timestamp,permalink&limit=20&access_token=${ACCESS_TOKEN}")

# Get account info
ACCOUNT=$(curl -s "https://graph.instagram.com/v18.0/${INSTAGRAM_USER_ID}?fields=id,username,account_type,media_count&access_token=${ACCESS_TOKEN}")
...
for item in data.get('data', []):
    post = {
        'id': item.get('id'),
        'caption': (item.get('caption', '')[:60] + '...') if len(item.get('caption', '')) > 60 else item.get('caption', ''),
